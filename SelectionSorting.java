...
public class SelectionSorting {
    ...
    // Starts from a specific index and goes through the entire array to find the samllest element ahead of the element at the index
    // If a smallest element is found, it is swapped with the element at the index
    // The index is then incremented by 1 and the process is repeated until the entire array is sorted
    public static int[] selectionSort(int[] arrayUnSorted) {
        int n = arrayUnSorted.length;
        for (int i = 0; i < n - 1; i++) {
            int minIndex = i;
            for (int j = i + 1; j < n; j++) {
                if (arrayUnSorted[j] < arrayUnSorted[minIndex]) {
                    minIndex = j;
                }
            }
            if (minIndex != i){
                arrayUnSorted = arrayElementSwap(arrayUnSorted, minIndex, i);
            }
        }
